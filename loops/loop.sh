#!/usr/bin/env bash
# loop.sh — outer loop for the Discovery PoC Orchestrator
#
# Usage: bash loops/loop.sh
# Runs Bob Shell iterations until all tasks in IMPLEMENTATION_PLAN.md are done
# or MAX_ITERATIONS is reached.

# Re-exec under bash if invoked via plain sh (e.g. "sh loops/loop.sh")
...
echo "======================================================"
echo "  Discovery PoC Orchestrator — Agentic Build Loop"
echo "======================================================"
echo ""
...
if [ "$CURRENT_BRANCH" = "main" ] || [ "$CURRENT_BRANCH" = "master" ]; then
    BRANCH_NAME="feature/poc-orchestrator-loop"
...
while true; do
    ...
    if [ -f "$PLAN_FILE" ]; then
        if ! grep -q "\- \[ \]" "$PLAN_FILE"; then
            ...
            echo "Discovery PoC Orchestrator build loop finished."
            break
        fi
    fi
    ...
    bash "$REPO_ROOT/loops/iteration.sh" "$ITERATION"
