#!/usr/bin/env bash
# iteration_v2.sh — runs one Bob Shell iteration for the Discovery PoC Orchestrator V2 loop
#
# Usage: bash loops/iteration_v2.sh [ITERATION_NUMBER]

# Re-exec under bash if invoked via plain sh
...
ITERATION="${1:-}"
PROMPT_FILE="$REPO_ROOT/loops/prompt_v2.md"
...
if [ -z "$ITERATION" ]; then
    LAST_COMMIT=$(git -C "$REPO_ROOT" log --all --grep="PoC Orchestrator V2 Iteration" --format="%s" -n 1 2>/dev/null || true)
...
# ── Commit changes ────────────────────────────────────────────────────────────
git -C "$REPO_ROOT" add .
git -C "$REPO_ROOT" commit -m "PoC Orchestrator V2 Iteration $ITERATION" --allow-empty
